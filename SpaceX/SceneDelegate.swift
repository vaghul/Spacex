...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, openURLContexts URLContexts: Set<UIOpenURLContext>) {
         if let url = URLContexts.first?.url {
             ...
             let path = component.first?.components(separatedBy: "://")
             if component.count > 1, let appId = component.last, let hostpath = path,hostpath.count > 1, hostpath.last == "launch"  {
                 if let topViewController = self.window?.rootViewController as? UINavigationController {
                     let detailVC = DetailViewController.prepare(from: .DetailViewController)
                     detailVC.modalPresentationStyle = .popover
                     detailVC.launchid = appId
